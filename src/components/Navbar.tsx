  ...
  return (
    <nav className="w-full flex md:px-20 px-2 py-6 justify-between items-center bg-snow z-30 navbar">
      ...
      <ul className='list-none sm:flex hidden justify-end items-center flex-1'>
        ...
        {/*  Cart */}
        <li className={`relative mr-10 ${active === 'cart' ? 'text-yellow' : 'text-primary'}`}
          onClick={() => setActive('cart')}>
            <span className={`absolute px-2 bg-yellow rounded-full left-4 bottom-5 ${active === 'cart' ? 'text-primary' : ''}`}>1</span>
          <Cart />
        </li>
      ...
      <div className='sm:hidden flex flex-1 justify-end items-center'>
        {/* cart on mobile view*/}
        <Link to="/cart" className={`relative mr-4 active:text-yellow ${active === 'cart' ? 'text-yellow' : 'text-primary'}`}
          onClick={() => setActive('cart')}>
            <span className={`absolute px-2 bg-yellow rounded-full left-4 bottom-5 ${active === 'cart' ? 'text-primary' : 'text-primary'}`}>10</span>
          ...
          <div className='p-6 bg-snow w-[60%] h-full shadow-lg sidebar z-400 relative'>
            <Link to='/'>
              <h2 className="text-2xl font-bold mb-8 text-center"
                ...
                Ec<span><img src={logo} alt="ecommerce" className="w-[28px] h-[36px] inline-block pb-2" /></span>mmerce</h2>
            </Link>

            <ul className='list-none flex flex-col mt-8 items-center flex-1'>
              <li className={`mb-10 ${active === 'search' ? 'text-yellow' : 'text-primary'}`}
                onClick={() => {
                  setActive('search');
                  setToggle(false);
                }}>
